...
SFR:: The TOE shall combine the input signals to generate a score that will be reported to the authentication system to determine the lock/unlock status of the device.
...
===== FPT_TST.1 TSF Testing (modified)

FPT_TST.1.1:: The TSF shall run a suite of self tests [selection: _during initial start-up, periodically during normal operation, at the request of the authorised user, at the conditions[assignment: conditions under which self test should occur]_] to demonstrate the correct operation of [_the TSF_].

*INFO:* The purpose of this is to have some sort of check on the generation of the output to ensure that it is properly functioning. This may not be necessary, but would seem to be tied to FPT_FLS.1 in terms of knowing the ysstem is functioning.

==== Trusted Path/Channels

* is this useful to show some paths to the CMFA engine? Trust?
FPT_CDP_EXT.1.2:: Transmission of plaintext biometric data between the capture sensor and the SEE shall be isolated from the main computer operating system on the TSF in runtime.
...
*INFO:* Trusted path here is to ensure a secure channel between the input signal and the CMFA system. Disclosure of the data is not really a concern here, though this could be useful from a privacy standpoint (again because the data collected for CMFA is really personal).